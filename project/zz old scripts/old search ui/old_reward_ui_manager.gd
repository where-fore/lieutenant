...
func search_for_rare_reward() -> void:
	...
	if roll <= rare_chance:
		current_search_reward = Database.get_rewards_by_category(Categories.item_rarity, [Categories.Rarity.RARE]).pick_random()
	elif roll <= rare_chance + common_chance:
		current_search_reward = Database.get_rewards_by_category(Categories.item_rarity, [Categories.Rarity.COMMON]).pick_random()
	else:
		pass
