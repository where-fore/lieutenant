...
func setup_item_stats() -> void:
	...
	extra_tooltip = "After attacking, for every {threshold} {stat},\ndeal damage equal to {percent}% of your {stat}".format({"threshold": threshold_per_hit, "stat": stat_to_check,  "percent": percent_stat_to_deal_bonus}) # "Generic flavourful description"
	reward_categories = {
...
var percent_stat_to_deal_bonus:int = percent_of_stat_already_used_for_attack / 2
...
func on_attack(source:Combatant, target:Combatant) -> void:
	...
	var damage_to_deal:int = source.current_stats[stat_to_check] * percent_stat_to_deal_bonus / 100
